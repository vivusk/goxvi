...
- `key-event-handler.*` — phân loại VK (map A–Z thủ công, KHÔNG ToUnicodeEx — M4); VK 0-9 → Letter chỉ khi `inputMethod=Vni`; CommitAndPass (terminator) tách riêng CommitAndPassChord (Ctrl/Alt/Win); compartment disabled + input scope password; `clearFocusedEditControlSelection` (`EM_GETSEL` + `WM_CLEAR` sync lên focused "edit" window — CUAS giấu selection thật khỏi GetSelection; inject VK_DELETE đã thử và fail: Explorer terminate composition non trẻ khi text đổi); `reinjectKey` (SendInput, EXTENDEDKEY chỉ cho nav key, tag `kGoxviInjectedKeyTag`).
- `key-event-sink.cpp` — OnTestKeyDown/OnKeyDown chung handler, eaten nhất quán; letter đầu từ (chỉ KeyDown, Test side-effect free) → `clearFocusedEditControlSelection` trước khi compose (fix gõ đè selection ở Explorer F2 rename/CUAS); terminator khi compose → eat → commit → reinject (fix thứ tự phím/chữ ở terminal + CUAS edit); chord commit idempotent chạy cả trong Test, không eat; Esc → `cancelWordToRaw`. `commitCurrentWord(applyShortcut=true)` là điểm hội tụ gõ tắt (7 call site; chỉ Esc truyền false).
...
- `tip-registration.*` — CLSID (HKCR, Apartment) + profile 0x042A + categories (không secure mode — L5).

Ghi chú lịch sử: direct input mode cho terminal đã thử rồi bỏ theo yêu cầu user
(muốn 1 cơ chế thống nhất) — xem git commit 13abb0c nếu cần lại.
...
- `src/main.cpp` — entry point, single-instance (mutex), DPI PerMonitorV2, khởi tạo comctl32; bắt cờ `--activate-tip` (chạy helper rồi thoát, trước mutex/cửa sổ).
- `src/activate-tip.{cpp,h}` — `Goxvi.exe --activate-tip`: helper không cửa sổ, TSF `ActivateProfile(0x042A, CLSID_GoxviTextService, GUID_GoxviProfile, TF_IPPMF_FORSESSION|TF_IPPMF_ENABLEPROFILE)` đặt Goxvi làm bộ gõ VIE của phiên user (checkbox trang cuối installer, chạy `runasoriginaluser`). DÙNG LẠI `tsf/src/goxvi-guids.cpp` (initguid TU) — COM-only, không `msctf.lib`, không chọc `HKCU\...\User Profile`.
